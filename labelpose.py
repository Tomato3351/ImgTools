...
class Objectpose:
    def __init__(self, label, kpt_shape, imgage_height, image_wdith):
        self.label = label
        self.kpt_shape  = kpt_shape
        self.imgage_height = imgage_height
        self.image_wdith  = image_wdith
        self.group_id = None        
    ...
    def read_from_str(self, yolo_str, group_id):
        ...
        for j in range(self.kpt_shape[0]):
            if(2==self.kpt_shape[1]):
                self.poses.append([float(str_list[5+j*self.kpt_shape[1]]),float(str_list[5+j*self.kpt_shape[1]+1])])
            elif(3==self.kpt_shape[1]):
                self.poses.append([float(str_list[5+j*self.kpt_shape[1]]),float(str_list[5+j*self.kpt_shape[1]+1]),float(str_list[5+j*self.kpt_shape[1]+2])])
            else:
                print("Warn:kpt_shape[1] must be 2 or 3")
        
    def rect2xyxy(self)->list:
        center_x, center_y, w, h = self.rect
        left_top_pt_x = center_x - w/2
        left_top_pt_y = center_y - h/2
        right_bottom_pt_x = center_x + w/2
        right_bottom_pt_y = center_y + h/2        
        return [left_top_pt_x, left_top_pt_y, right_bottom_pt_x, right_bottom_pt_y]
...
class CanvasImage:
    ...
    def __init__(self, placeholder, path):
        ...
        self.canvas.grid(row=0, column=0, rowspan=1, sticky='nswe')
    ...
    def unbindB1(self):
        ...
        self.canvas.unbind('<B1-Motion>')  # move canvas to the new position        
    def __bindB1(self):
        self.canvas.bind('<ButtonPress-1>', self.__move_from)  # remember canvas position
        self.canvas.bind('<B1-Motion>',     self.__move_to)  # move canvas to the new position

    def move_create_rect(self,event):#十字线
        x = self.canvas.canvasx(event.x)  # get coordinates of the event on the canvas
        y = self.canvas.canvasy(event.y)
        
        
        if(self.line_hv_inited):
            self.canvas.delete(self.line_h)
            self.canvas.delete(self.line_v)        
        
        self.line_h = self.canvas.create_line(0,y,self.imwidth,y,fill="black", width=1,dash=(2, 2))
        self.line_v = self.canvas.create_line(x,0,x,self.imheight,fill="black", width=1,dash=(2, 2))
    ...
    def create_rect_2p(self, event):
        ...
        if self.click_cnt==kpt_shape[0]+2:
            ...
            print("obj_pose = ",obj_pose.pose2yolostr())
            self.save_txt()
            self.new_group_id+=1
            self.__bindB1()
...
def delete_obj_back(event):
    global canvas
    if event.keycode==119 and len(canvas.obj_pose_list)>0:
        canvas.pop_obj_pose()
        canvas.save_txt()
        on_select_lb_fname(event)
...
def load_img_txt(name):
    global canvas, input_dir, canvas_init
    name_full = os.path.join(input_dir,name)
    txt_name = name.split(".")[0]+".txt"
    if canvas_init:
        canvas.destroy()
    canvas = CanvasImage(MainWindow, name_full)  # create widget
    canvas_init = True
    canvas.txt_name = os.path.join(input_dir,txt_name)
    canvas.grid(row=0, column=0, rowspan=4)
    if os.path.exists(canvas.txt_name):
...
def on_select_lb_fname(event):
    global canvas, input_dir
    selected_indices = file_name_listbox.curselection()[0]
    selected_file_name = file_name_listbox.get(selected_indices)
    load_img_txt(selected_file_name)

def previous_img():
    global canvas, input_dir
    selected_indices = file_name_listbox.curselection()[0]
    if selected_indices>0:
        file_name_listbox.selection_clear(0,file_name_listbox.size())
        file_name_listbox.selection_set(selected_indices-1)
        selected_file_name = file_name_listbox.get(selected_indices-1)
        load_img_txt(selected_file_name)
    
def next_img():
    global canvas, input_dir
    selected_indices = file_name_listbox.curselection()[0]
    if selected_indices+1<file_name_listbox.size():
        file_name_listbox.selection_clear(0,file_name_listbox.size())
        file_name_listbox.selection_set(selected_indices+1)
        selected_file_name = file_name_listbox.get(selected_indices+1)
        load_img_txt(selected_file_name)    
...
if __name__ == '__main__':
    ...
    MainWindow = tk.Tk()
    MainWindow.title('label')
    MainWindow.geometry('800x600')  # size of the main window
    MainWindow.rowconfigure(0, weight=1)  # make the CanvasImage widget expandable
    MainWindow.columnconfigure(0, weight=1)
    canvas_init = False
    btn_opendir = tk.Button(MainWindow, text='openDir', font=('Arial', 12), width=16, height=1, command=open_dir)
    ...
    btn_next = tk.Button(MainWindow, text='->', font=('Arial', 12), width=8, height=1, command=next_img)
    btn_next.grid(row=2, column=2, sticky='we')
            
    btn_create = tk.Button(MainWindow, text='create', font=('Arial', 12), width=16, height=1, command=create_obj)
    btn_create.grid(row=3, column=1, columnspan=2, sticky='we')

    MainWindow.bind('<Control-r>', create_obj_ctrlr)
    MainWindow.bind("<Key>",delete_obj_back)
    MainWindow.mainloop()
